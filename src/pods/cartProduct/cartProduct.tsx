...
import * as classes from './cartProduct.styles';
import { ProductInfo } from 'core/api';

interface Props {
  product: ProductInfo;
  sidebarState: number;
  onProductChecked: (productId: number) => void;
}

export const CartProduct: React.FunctionComponent<Props> = ({
  product,
  sidebarState,
  onProductChecked,
}) => {
  ...
  const { id, picUrl, title } = product;

  const onProductCheckedCurried = () => id => onProductChecked(id);

  return (
    <>
      <div className={className.productCart}>
        <div>
          <img src={picUrl} alt={`${title}_${id}`} />
        </div>
        {sidebarState === 1 && <h2>{title}</h2>}
        <span
          className={
            sidebarState === 1
              ? className.deleteProductExpanded
              : className.deleteProductCollapsed
          }
          //  Review
